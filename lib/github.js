...
import { scrypt, scryptAsync } from "@noble/hashes/scrypt.js"; // エクステンションを指定するか
...
export function getRepositoryInfo() {
  ...
  const info = localStorage.getItem("repositoryInfo");
  const _password = localStorage.getItem("password");
  const _salt = localStorage.getItem("salt");
  if (!info) return null;

  try {
    const _j = JSON.parse(info);
    _j.password = _password
    _j.salt = _salt
    return _j
  } catch (e) {
    console.error("repositoryInfo のパースに失敗しました", e);
    return null;
  }
}


export async function faileGet(filePath, octokit, repositoryInfo) {
  try {
...
export async function decryptBinFile(file, repositoryInfo) {
  ...
  const password = new TextEncoder().encode(repositoryInfo.password)
  const salt = new TextEncoder().encode(repositoryInfo.salt)
